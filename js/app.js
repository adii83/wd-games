document.addEventListener('DOMContentLoaded', () => {
    ...
    const hddDropdownList = hddDropdown ? hddDropdown.querySelector('.dropdown-list') : null;

    // Landing page elements
    const landingScreen = document.getElementById('landing-screen');
    const landingListBtn = document.getElementById('landing-list-btn');
    const landingStorageBtn = document.getElementById('landing-storage-btn');
    const landingBackBtn = document.getElementById('landing-back-btn');
    const landingMainActions = document.getElementById('landing-main-actions');
    ...
    const STORAGE_PRESETS = {
        ...
        flashdisk: [
            ...
            { label: '256 GB', value: 232 }
        ]
    };

    let currentStorageType = 'hdd';
    
    // Initial capacity follows default HDD preset.
    let currentHddCapacity = STORAGE_PRESETS[currentStorageType][1].value;
    let totalUsedGB = 0;
    ...
    function getStorageLabel(type) {
        if (type === 'flashdisk') return 'FLASHDISK';
        return String(type || 'hdd').toUpperCase();
    }

    function renderStorageCapacityOptions(storageType, preferredValue) {
        const safeType = STORAGE_PRESETS[storageType] ? storageType : 'hdd';
        const options = STORAGE_PRESETS[safeType];
        const selected = options.find((opt) => opt.value === preferredValue) || options[0];

        currentStorageType = safeType;
        currentHddCapacity = selected.value;

        if (storageTypeLabelEl) {
            storageTypeLabelEl.innerText = getStorageLabel(safeType);
        }

        if (hddSelectedText) {
            hddSelectedText.innerText = `${getStorageLabel(safeType)} ${selected.label}`;
        }

        if (hddDropdownList) {
            hddDropdownList.innerHTML = options
                .map((opt) => `<li class="dropdown-item${opt.value === selected.value ? ' active' : ''}" data-value="${opt.value}">${opt.label}</li>`)
                .join('');
        }
    }

    function closeLandingAndEnter() {
        if (landingScreen) {
    ...
    function showLandingStorageChoices(show) {
        if (!landingMainActions || !landingStorageActions) return;
        landingMainActions.style.display = show ? 'none' : 'grid';
        landingStorageActions.style.display = show ? 'grid' : 'none';
    }

    if (landingListBtn) {
        landingListBtn.addEventListener('click', () => {
            closeLandingAndEnter();
        });
    }

    if (landingStorageBtn) {
        landingStorageBtn.addEventListener('click', () => {
            showLandingStorageChoices(true);
        });
    }

    if (landingBackBtn) {
        landingBackBtn.addEventListener('click', () => {
            showLandingStorageChoices(false);
        });
    }
    ...
    if (landingStorageActions) {
        landingStorageActions.addEventListener('click', (e) => {
            ...
            const nextType = btn.getAttribute('data-storage');
            renderStorageCapacityOptions(nextType);
            updateStorageUI();
            closeLandingAndEnter();
        });
    }

    renderStorageCapacityOptions(currentStorageType, currentHddCapacity);
    ...
    if (hddDropdownList) {
        hddDropdownList.addEventListener('click', (e) => {
            const item = e.target.closest('.dropdown-item');
            if (!item) return;

            hddDropdownList.querySelectorAll('.dropdown-item').forEach((i) => i.classList.remove('active'));
            item.classList.add('active');

            if (hddSelectedText) {
                hddSelectedText.innerText = `${getStorageLabel(currentStorageType)} ${item.innerText}`;
            }
